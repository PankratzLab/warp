...
task PairedFastqsToBwaMem {
  ...
  Float fastqs_size = size(read1, "GiB") + size(read2, "GiB")
  ...
  Int disk_size = ceil(fastqs_size + bwa_ref_size + (disk_multiplier * fastqs_size) + 20)
  ...
  command <<<
    ...
    if [ -s ~{reference_fasta.ref_alt} ] || ~{allow_empty_ref_alt}; then
      /usr/gitc/~{bwa_commandline} > ~{output_bam_basename}.bam 2> ~{output_bam_basename}.bwa.stderr.log
