...
task CheckSamplesUnique {
  input {
    ...
    Int sample_num_threshold = 50 # BI best practice threshold = 50
  }
...
task ImportGVCFs {
  ...
  command <<<
    ...
    gatk --java-options "-Xms8000m -Xmx50000m -XX:+UseStringDeduplication -DGATK_STACKTRACE_ON_USER_EXCEPTION=true" \
      GenomicsDBImport \
...
task SNPsVariantRecalibrator {
  ...
  Int auto_mem = ceil(2 * size([sites_only_variant_filtered_vcf,
                              ...
                              dbsnp_resource_vcf],
                      "MiB"))
  Int machine_mem_mib = select_first([machine_mem_mb, if auto_mem < 7000 then 7000 else auto_mem])
  Int java_mem = machine_mem_mib - 1000
  Int max_heap = machine_mem_mib - 500

  # Convert mebibytes to gigabytes for Cromwell
  Int machine_mem = ceil(machine_mem_mib * 0.001048576)
...
task CrossCheckFingerprint {
  ...
  Int disk = 100
  Int mem_gb = ceil(memMb * 0.001048576)
